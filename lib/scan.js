...
const { scanImports } = require("./casecheck");
...
function walk(dir, exts, files = [], skipped = null) {
  ...
  for (const e of entries) {
    ...
    } else if (exts.includes(path.extname(e.name)) || e.name === "package.json") {
      // package.json is scanned too: the `scripts` block is where the most
      // common Windows bug in the ecosystem actually lives.
      files.push(full);
    }
  }
  return files;
...
function scanFile(file, opts = {}) {
  ...
  const findings = scanSource(src, file, opts);

  // The case check needs the filesystem, so it cannot live in core.js with the
  // text rules -- and for the same reason it cannot run in the browser
  // checker. It is skipped for package.json, which has no imports.
  if (!/(^|[\/])package\.json$/i.test(file)) {
    try {
      findings.push(...scanImports(src, file));
    } catch {
      // A rule that cannot read a directory must not take the whole scan down.
    }
  }
